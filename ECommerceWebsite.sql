CREATE DATABASE IF NOT EXISTS ECommerceWebsite;
USE ECommerceWebsite;

/*1.create tables for supplier,customer,category,product,supplier_pricing,order,rating to store the data for the E-commerce */
...
SELECT  COUNT(CUSTOMER.CUS_ID) AS CUSTOMER_ID, CUSTOMER.CUS_GENDER 
FROM CUSTOMER,ORDERS 
WHERE ORDERS.CUS_ID = CUSTOMER.CUS_ID AND ORDERS.ORD_AMOUNT >= 3000
GROUP BY CUS_GENDER;

/*4.Display all the orders along with product name ordered by a customer having Customer_Id=2*/
SELECT ORDERS.*, PRODUCT.PRO_NAME
FROM PRODUCT,SUPPLIER_PRICING,ORDERS
WHERE ORDERS.PRICING_ID = SUPPLIER_PRICING.PRICING_ID AND SUPPLIER_PRICING.PRO_ID = PRODUCT.PRO_ID AND ORDERS.CUS_ID = 2;
...
SELECT ORDERS.*, PRODUCT.PRO_ID,PRODUCT.PRO_NAME FROM ORDERS
...
begin
	SELECT  SUPPLIER.SUPP_ID,SUPPLIER.SUPP_NAME,AVG(RATING.RAT_RATSTARS) AS AVERAGE_RATING,
	CASE 
	...
	from RATING,SUPPLIER_PRICING,SUPPLIER,ORDERS
    WHERE RATING.ORD_ID = ORDERS.ORD_ID AND ORDERS.PRICING_ID = SUPPLIER_PRICING.PRICING_ID AND SUPPLIER_PRICING.SUPP_ID = SUPPLIER.SUPP_ID
